...
public class ParkingSpace {
    ...
    private final int spotType;   //1=express  0=regular
    private final List<Vehicle> parkingVehicles = new ArrayList<>();

    //ParkingSpace constructor

    public ParkingSpace(int spotType) {

        this.spotType = spotType;
    }

    public boolean accepts(Vehicle vehicle) {
        return isSpaceAvail(() -> (spotType >= parkingVehicles.get(0).getSpacesNeeded() + vehicle.getSpacesNeeded()));
    }
    ...
    public boolean isFree() {
        return isSpaceAvail(() -> (spotType > parkingVehicles.get(0).getSpacesNeeded()));
    }
    ...
    public String toString() {
        return "Slot [id: " + id + ", availableSpots=" + spotType + ", numofParkingVehicles=" + parkingVehicles + "]";
    }
}
